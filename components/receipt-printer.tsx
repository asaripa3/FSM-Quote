...
import type { QuoteExclusion, QuoteLine } from "@/lib/quote-pdf";

export function ReceiptPrinter({ pack, lines, quote, onClose, company = "", exclusions = [] }: { pack: TradePack; lines: QuoteLine[]; quote: ReturnType<typeof buildQuote>; onClose: () => void; company?: string; exclusions?: QuoteExclusion[] }) {
  ...
  const download = async () => {
    ...
    try { const { downloadQuotePdf } = await import("@/lib/quote-pdf"); await downloadQuotePdf(pack, lines, quote, company, exclusions); }
  ...
  return <dialog ref={dialogRef} className="receipt-dialog" aria-labelledby="receipt-title" onCancel={onClose} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
    <div className="receipt-modal-content"><div className="receipt-heading"><div><span className="micro-label">THE DELIVERABLE</span><h2 id="receipt-title">Your quote, on paper.</h2></div><button className="receipt-close" onClick={onClose} aria-label="Close quote receipt" autoFocus>×</button></div>
