...
import { validAmount, type JobSettings, type ParsedJob, type PickedSource, type Discovery, type ResolvedPart, type ExaTrace } from "@/lib/job";
...
export function Workflow({pack}:{pack:TradePack}) {
 ...
 useEffect(()=>{
  ...
  let cancelled=false;
  // Sequential rather than parallel: each item costs two Exa calls, and a run should stay predictable.
  (async()=>{ for(const part of parts){ if(cancelled) return; await findSources(part); } })();
    ...
    {discovery&&!busy&&<div className={showExa?"exa-on":"exa-off"}>
     {showExa?<>
      ...
      <PartsCart discovery={discovery} searches={searches} picks={picks} quantities={quantities}
       onPick={(id,pick)=>setPicks(s=>{const next={...s};if(pick)next[id]=pick;else delete next[id];return next;})}
       onQuantity={(id,value)=>setQuantities(q=>({...q,[id]:value}))}
       onRemove={dropResolved}
