...
export function Workflow({pack}:{pack:TradePack}) {
 ...
 const analyze=async(confirmation?:Confirmation)=>{
  ...
  setError("");setBusy(true);setStage("parsing");setRoutes(null);setPhase(confirmation?"act":"research");
 ...
 return <div className="decision-workspace">
  <header className="dw-top"><div><p className="dw-eyebrow">{pack.name} workspace</p><strong>Field observation to confirmed repair</strong></div><div className="dw-toolbar"><button className="dw-text-button" aria-pressed={showExa} onClick={()=>setShowExa(v=>!v)}>Research activity</button><button className="dw-button" onClick={()=>setShowSettings(true)}>Rates & suppliers</button></div></header>
  ...
  <div className={`dw-layout${phase==="act"?" dw-act":""}`}>
   <aside className="dw-context"><p className="dw-eyebrow">Current job</p><h2>{job?.brief.equipment||`${pack.name} inspection`}</h2><p className="dw-small">{job?.brief.manufacturer||pack.name}</p>
   ...
   {phase==="capture"&&<><section className="dw-main dw-capture"><p className="dw-status">{job?"Update your observation":"New field observation"}</p><h1>What are you seeing?</h1><p className="dw-copy">The equipment, the symptom, and what you have already checked.</p>
    <div className="dw-tabs" role="group" aria-label="Input method">{([["note","Type a note"],["audio","Upload audio"],["mic","Use microphone"]] as const).map(([id,label])=><button key={id} aria-pressed={mode===id} onClick={()=>setMode(id)}>{label}</button>)}</div>
    {mode!=="note"&&!busy&&<AudioInput key={mode} mode={mode} onTranscript={appendTranscript}/>}
    <label className="dw-label">{mode==="note"?"Field observation":"Transcript — review before research"}<textarea value={note} onChange={e=>setNote(e.target.value)} disabled={busy} rows={6} maxLength={12000} placeholder="Equipment is doing… I checked… I’m still unsure about…"/></label>
    <details className="dw-disclosure"><summary>Add equipment model or serial</summary><label className="dw-label">From the equipment plate<input value={plate} onChange={e=>setPlate(e.target.value)} maxLength={120}/></label></details>
    <div className="dw-action-bar"><span className="dw-small">{job?"Researching again replaces this job’s current selections.":"Customer and quote details come later."}</span><button className="dw-button dw-primary" disabled={busy||note.trim().length<12} onClick={()=>analyze()}>Research this job <span aria-hidden="true">→</span></button></div>
   </section><aside className="dw-inspector"><p className="dw-eyebrow">What happens next</p><h2>A check you can act on.</h2><ol className="dw-intro-list"><li><strong>Research the equipment</strong><span>Manufacturer documentation and field knowledge.</span></li><li><strong>Confirm your finding</strong><span>You decide what the on-site check established.</span></li><li><strong>Source and quote</strong><span>Parts only when needed. Your shop’s rates.</span></li></ol></aside></>}
  ...
  {showExa&&<div className="dw-activity"><ExaResearch events={events} trace={fullTrace} busy={busy} routes={routes} onCancel={cancelResearch}/></div>}
  <footer className="dw-footer"><span>Research by Exa · Decisions by your technician</span><span>{packet?`${packet.sources.length} sources retained`:"Evidence stays with the job"}</span></footer>
