...
export function Nav({ context }: { context?: string }) {
 ...
 return <header ref={ref} className={`site-header${overSky ? " over-sky" : ""}`}><div className="page-width nav-inner"><BrandLink className="site-brand"><ExaLogo /><span className="brand-divider" /><strong>FSMpedia<span className="brand-cross">✳</span></strong></BrandLink><nav aria-label="Main navigation"><Link href="/#how">How it works</Link><Link href="/#why">Why FSMpedia</Link></nav>{context ? <div className="face-switcher" aria-label="Choose trade">{TRADE_LIST.map(t => <Link href={`/workflow/${t.id}`} key={t.id} aria-label={`Switch to ${t.name}`} aria-current={context === t.name ? "page" : undefined} title={t.name}><span><Image src={t.mascot} alt="" width={t.mascotW} height={t.mascotH} unoptimized /></span></Link>)}</div> : <Link className="nav-cta" href="/#packs">Start a job <span>↗</span></Link>}</div></header>;
}
export function Footer() { return <footer className="site-footer page-width"><BrandLink className="site-brand"><ExaLogo /><span className="brand-divider" /><strong>FSMpedia</strong></BrandLink><span>The live knowledge layer for field technicians.</span><span className="micro-label">BUILT FOR THE FIELD</span></footer>; }
